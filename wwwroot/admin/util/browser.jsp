...
//Change this to locate the tempfile directory for upload (not longer needed)
	...
	static Vector expandFileList(String[] files, boolean inclDirs) throws Exception {
		Vector v = new Vector();
		if (files == null) return v;
		for (int i=0; i < files.length; i++) v.add (new File(URLDecoder.decode(files[i], "UTF-8")));
	...
	*/
	static String dir2linkdir(String dir, String browserLink, int sortMode) throws Exception{
		File f = new File(dir);
		StringBuffer buf = new StringBuffer();
		while (f.getParentFile()!=null){
			if (f.canRead()){
				String encPath = URLEncoder.encode(f.getAbsolutePath(), "UTF-8");
...
<%
	...
	<HR>
	<CENTER>
		<SMALL>jsp File Browser version <%= VERSION_NR%></SMALL>
	</CENTER>
</BODY>
</HTML><%
	}
