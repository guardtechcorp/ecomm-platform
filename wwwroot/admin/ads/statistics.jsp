...
<%@ page import="com.zyzit.weboffice.bean.AdsStatisticsBean" %>
<%@ page import="com.zyzit.weboffice.model.UserInfo" %>
<%@ page import="java.util.List" %>
<%
  AdsStatisticsBean bean = new AdsStatisticsBean(session, 10);
//bean.showAllParameters(request, out);
  if (!bean.canAccessPage(request, response, AccessRole.ROLE_ADS_STATISTICS))
     return;

  UserInfo urInfo = bean.getLoggedUserInfo();
//  int nSpecificUserId = Utilities.getInt(request.getParameter("userid"), 0);
  ...
  if ("Show Statistics Report".equalsIgnoreCase(sAction))
  {
    ...
    if (nTotalRecords>0)
    {
       ...
       response.sendRedirect("reportlist.jsp?action=Show Report");
    }
    else
    {
  ...
  String sTitleLinks = "<b>Statistics Report</b>";
%>
<SCRIPT Language="JavaScript">setUpArea('<%=sHelpTag%>', '<%=sTitleLinks%>');</SCRIPT>
This page will allow you to define a date range to get a overview of statistic report summary.
<p>
...
<table border="0" cellpadding="3" cellspacing="1" width="98%" class="forumline" align="center">
  <form name="salereport" action="statistics.jsp"  method="post" onSubmit="validateDisplay(this);showHideSpan('open','Processing');">
...
<% } %>
    ...
    <tr>
      <td class="row1" align="right" valign="top">Users:</td>
      <td class="row1" height="19">
      <select name="userid" size="6" multiple>
<%
       List ltUser = bean.getUserList(urInfo, nUserId);
       for (int i=0; i<ltUser.size(); i++) {
          UserInfo info = (UserInfo)ltUser.get(i);
          String sName = info.RealName;
          if (Utilities.getValueLength(sName)==0)
             sName = info.Username;
%>
       <option value="<%=info.UserID%>"><%=Utilities.getValue(sName)%></option>   
<% } %>
      </select>
      </td>
      <td class="row1" colspan="2" valign="top">Hold down the Shift or Ctrl key and click on the items you want to select or deselected. <br>If no selection, it means all users.</td>
      <td class="row1" width="29%" valign="top">Show:
        <select name="showtype">
          <option value="all">All Together</option>
          <option value="each" selected>Each Ads</option>
        </select>
      </td>
    </tr>
    <tr>
