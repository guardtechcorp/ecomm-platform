...
<%
  ...
  else if ("Delete".equalsIgnoreCase(sAction))
  {
    ...
    if (nTotalRecords<0)
       sDisplayMessage = "Delete the order was fialed, Please try again later.";
  }
  else if ("Stop".equalsIgnoreCase(sAction)||"Active".equalsIgnoreCase(sAction))
  {
    int nRet = bean.StopActive(request, sAction);
    if (nRet<=0)
      sDisplayMessage = "Stop or Active the order was fialed, Please try again later.";
  }
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
  <tr>
   <td width="20%"><a href="ordersearch.jsp?action=Search Order&return=<%=Utilities.getUrlEncode("orderlist.jsp|Orders")%>&display=Order List">Search Orders</a></td>
...
<tr class="normal_row">
  ...
  <td width="21%"><a href="../util/email.jsp?action=person&toemail=<%=info.EMail%>&name=<%=info.Yourname%>&group=recipients&return=<%=Utilities.getUrlEncode("../customer/customerlist.jsp|Customers")%>&display=Customer List" title="Email to <%=info.EMail%>"><%=info.EMail%></a></td>
  <td width="13%"><%=bean.getShortModules(info)%></td>
  <!--td width="10%"><%=info.OrderNo%></td-->
  <td width="16%"><a href="order.jsp?action=view&orderid=<%=info.OrderID%>&customerid=<%=info.CustomerID%>&return=<%=Utilities.getUrlEncode("../order/orderlist.jsp|Orders")%>&display=Order List"><%=info.TrackNo%></a></td>
