...
function onCustomerAccountLoad(form, nCustomerId)
{
  if (nCustomerId<=0)
  {
...
function validateCustomerAccount(form)
{
  //. check the your name    
...
function onClientSignInLoad(form)
{
  ...
  else
  {
     ...
     form.rememberaccount.checked = true;      
     setFocus(form.password);
  }
}

function validateClientSignIn(form, value)
{
  //1. check the email
  if (checkFieldEmpty(form.email))
  {
  ...
  if (form.rememberaccount.checked)
     setCookie('account_email', form.email.value);
  else
     eraseCookie('account_email');
  return true;
}
...
//----------------------------
function onCustomerBillingLoad(form)
{
...
function validateCustomerBilling(form)
{
...
function setCountryCheckboxList()
{
  for (var i=0; i<g_Country.length; i++)
  {
     var sCountry = '';
     if (i>0)
        sCountry = '<br>';
     sCountry += '<input type="checkbox" name="country_'+ g_Country[i][0] +'" value="'+ g_Country[i][0]+ '">' + g_Country[i][1];
     document.write(sCountry);
  }
}
