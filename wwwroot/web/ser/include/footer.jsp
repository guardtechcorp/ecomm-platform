...
<% if (mcBean.getMemberInfo()==null && (cfInfo.AdvertiseBar&1)!=(byte)1) { %>
&nbsp;&nbsp; [ <A class='content' href='<%=mcBean.encodedUrl("index.jsp?action=Load_SignIn")%>'><%=mcBean.getLabel("cm-signin")%></A> | <A class='content' href='<%=mcBean.encodedUrl("index.jsp?action=Load_SignUp&rl=1&pt=Sign Up Informaton")%>'><%=mcBean.getLabel("cm-signup")%></A> ]
<% } %>
</DIV>
<script language="javascript">
