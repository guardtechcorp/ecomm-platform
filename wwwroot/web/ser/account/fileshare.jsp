...
/*
    else if (sRealAction.startsWith("SubmitSignIn")) {//. Try to login
        info = (ShareAccessInfo) bean.getLastInfo();
        MemberAccountBean maBean = MemberAccountBean.getObject(session);
        ret = maBean.signIn(request);
        if (!ret.isSuccess()) {
        ...
        {
           if (info.AccessMode==Definition.ACCESSMODE_LOGINUSER)
              bCanDownload = true;
           else if (info.AccessMode==Definition.ACCESSMODE_MYSITEMEMBER)
           {
              MemberInfo userInfo = (MemberInfo)ret.m_UpdateInfo;
              if (bean.isMemberOfSite(userInfo.MemberID))
                 bCanDownload = true;
              else
              {
                 sDisplayMessage = "You are successfully signed in, but you are not the member of this website. You have to apply its membership first first.";
                 nDisplay = 0;
              }
           }
        }
    } else if (sRealAction.startsWith("SubmitUnlockCode")) { //. Check the unlock code
        info = (ShareAccessInfo) bean.getLastInfo();
        if (!info.UnlockCode.equals(request.getParameter("unlockcode"))) {
            nDisplay = 0;
            sDisplayMessage = "The unlock code you entered is wrong, please try another one.";
        } else
            bCanDownload = true;
    }
*/
    ...
    if (ret.isSuccess() && info!=null)
    {
        if (info.AccessMode == Definition.ACCESSMODE_PUBLIC) {
            ...
            sbMessage.append("<UL><LI>If you are not a user in <b>WebCetner</b> network, please <A class='focuslink' style='font-weight1:bold' href='" + mcBean.encodedUrl("index.jsp?action=Load_SignUp&rootlink=yes&pagetitle=Sign Up Informaton") + "'>Sign Up</A> now</LI><p>");
            ...
            sbMessage.append("<UL><LI>If you are not a user in <b>WebCetner</b> network, please <A class='focuslink' style='font-weight1:bold' href='" + mcBean.encodedUrl("index.jsp?action=Load_SignUp&rootlink=yes&pagetitle=Sign Up Informaton&apply=1") + "'>Sign Up</A> ");
...
<% if (info.AccessMode==Definition.ACCESSMODE_PUBLIC) { %>
  <tr>
   <td colspan="2" align="center"><input type="button" value="Download" name="submit" onClick="return startDownload(document.form_sharefile, '<%=bean.encodedUrl("ajax/response.jsp")%>');" style="width:90px; height:22px"></td>
  </tr>
<% } else if (info.AccessMode==Definition.ACCESSMODE_MYSITEMEMBER||info.AccessMode==Definition.ACCESSMODE_LOGINUSER) { %>
  ...
  <tr>
    ...
    <td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="button" value="Sign In" name="sumbit" onClick="return signInDownload(document.form_sharefile, '<%=bean.encodedUrl("ajax/response.jsp")%>');" style="width:90px; height:22px">
    </td>
  </tr>
  <tr>
...
<% } else { %>
  <tr>
   <td colspan="2" align="center"><font size=2><b>Please enter an unlook code to download it:</b></font></td>
  </tr>
  <tr>
   <td width="30%" align="right">Unlook Code:</td>
   <td>
...
%>
  <tr>
   <td colspan="2" align="center"><input type="button" value="Submit" name="sumbit" onClick="return unLockDownload(document.form_sharefile, '<%=bean.encodedUrl("ajax/response.jsp")%>');" style="width:90px; height:22px"></td>
  </tr>
